...
class AmountToWords {
  ...
  ///
  /// The [number] argument must be a int representing the amount in Indian currency.
    ...
    while (i < digitsLength) {
      ...
      if (currentNumber > 0) {
        ...
        str.add((currentNumber < 21)
            ? '${_words[currentNumber]} ${_digits[str.length]} $hundred'
            : '${_words[(currentNumber / 10).floor() * 10]} ${_words[currentNumber % 10]} ${_digits[str.length]} $hundred');
      } else {
        str.add('');
      }
    }
      ...
      while (j < paisaLength) {
        ...
        if (currentNumber > 0) {
          ...
          str2.add((currentNumber < 21)
              ? '${_words[currentNumber]} ${_digits[str2.length]} $hundred'
              : '${_words[(currentNumber / 10).floor() * 10]} ${_words[currentNumber % 10]} ${_digits[str2.length]} $hundred');
        } else {
          str2.add('');
        }
      }
